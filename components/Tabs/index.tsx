    ...
    return (
        <div className={`flex flex-wrap -ml-1 ${className}`}>
            {items.map((item, index) => (
                <button
                    className={`h-8 ml-1 px-5 rounded-sm whitespace-nowrap text-xs font-bold transition-colors outline-none tap-highlight-color hover:text-muted dark:hover:text-purple-1 ${
                        value === item.value
